...
class IOWrapperParameter(Parameter):
    def __init__(self):
        super().__init__("IOWrapper", "--ioengine", [ 'LIBPMEM2_PF' ])
    ...
    def execute(self):
        ...
        for comb in filter(isvalidcomb, combinations):
            ...
            for thread_count in self.thread_counts:
                ...
                prep_cmds = [cmd.replace("_PF", "") for cmd in cmds]
                for _ in range(self.num_runs_per_configuration):
                    test = Test(cmds, preparation_cmds=prep_cmds)
                    test.run()
                    ...
                    if row[2].find("tmpfs") == -1:
                        row[2] = "pmem" if mounts[0].find("nvme") == -1 else "nvme"
                    else:
                        row[2] = "tmpfs"
